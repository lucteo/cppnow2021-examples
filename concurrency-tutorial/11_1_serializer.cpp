...
void buggy_software() {
    ...
    auto producer_process = [&]() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            // Wait for a bit
            sleep_in_between_ms(7, 30);
            // Produce one value
            double val = fortune_teller(crystal_ball);

            // Push the value into our average window object
            my_window.add(val);
        }
    };
    ...
    auto average_consumer_process = [&]() {
        ...
        for (int i = 0; i < 100; i++) {
            // Wait for a bit
            sleep_in_between_ms(10, 30);
            // Try to get a value and report it
            double val = my_window.get_average();
            printf("%5.2f\n", val);
            fflush(stdout);
        }
    };
